...
* Added independent SHT `lmax` / `mmax` limits and an optional non-inclusive `lmmax` bandwidth for generalized pentagonal truncation. Retained modes satisfy `m < mmax`, `m <= l < lmax`, and `l - m < lmmax` when provided; `lmmax=None` applies no bandwidth cap. Standard R42 uses `lmax=85, mmax=43, lmmax=43`, and a smaller independent `lmax` caps the rhomboid. `truncate_sht` returns an immutable `(lmax, mmax, lmmax)` descriptor. `SpectralConvS2` and `DistributedSpectralConvS2` retain their existing equal degree/order limits and behavior.
