...
* Added opt-in trapezoidal and rhomboidal SHT truncation. Rhomboidal truncation requires explicit `lmax` and `mmax` and retains modes with `l - m <= lmax - mmax`; for example, standard R42 uses `lmax=85` and `mmax=43`. Triangular truncation remains the default. `SpectralConvS2` and `DistributedSpectralConvS2` are unaffected.
